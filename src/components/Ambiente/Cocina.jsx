...
const Cocina = () => {
  const [ambientesCocina, setAmbientesCocina] = useState([]);

  useEffect(() => {
    async function fetchAmbientesCocina() {
      try {
        const { data, error } = await supabase
          .from('products')
          .select('*')
          .eq('ambientacion', 'Cocina');

        if (error) {
          throw error;
        }

        setAmbientesCocina(data || []);
      } catch (error) {
        console.error('Error al obtener los ambientes de cocina:', error.message);
      }
    }

    fetchAmbientesCocina();
  }, []);

  return (
    <div>
      {ambientesCocina.map(ambiente => (
