...
const Exterior = () => {
  const [ambientesExterior, setAmbientesExterior] = useState([]);

  useEffect(() => {
    async function fetchAmbientesExterior() {
      try {
        const { data, error } = await supabase
          .from('products')
          .select('*')
          .eq('ambientacion', 'Exterior');

        if (error) {
          throw error;
        }

        setAmbientesExterior(data || []);
      } catch (error) {
        console.error('Error al obtener los ambientes de exterior:', error.message);
      }
    }

    fetchAmbientesExterior();
  }, []);

  return (
    <div>
      {ambientesExterior.map(ambiente => (
