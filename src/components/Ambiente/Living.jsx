...
const Living = () => {
  const [ambientesLiving, setAmbientesLiving] = useState([]);

  useEffect(() => {
    async function fetchAmbientesLiving() {
      try {
        const { data, error } = await supabase
          .from('products')
          .select('*')
          .eq('ambientacion', 'Living');

        if (error) {
          throw error;
        }

        setAmbientesLiving(data || []);
      } catch (error) {
        console.error('Error al obtener los ambientes de living:', error.message);
      }
    }

    fetchAmbientesLiving();
  }, []);

  return (
    <div>
      {ambientesLiving.map(ambiente => (
